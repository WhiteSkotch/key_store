...
class Cart(object):
    ...
    def __iter__(self):
        ...
        # keys = Key.objects.filter(id__in=game_ids, is_sold=False)
        cart = self.cart.copy()
        for game in games:
            cart[str(game.id)]['game'] = game
        # for key in keys:
        #     cart[str(key.id)]['key'] = key
        ...
        for game_id, item in cart.items():
            ...
            # item['key'] = Key.objects.filter(game=game_id)[0]
            # item['key'] = cart[str(key.id)]['key'].key
            yield item
